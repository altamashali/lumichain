...
contract('LumiToken', function(accounts){
    ...
    it('initializes the contract with the correct values', function(){
        return LumiToken.deployed().then(function (instance){
            ...
            assert.equal(name, 'Lumi Token', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function(symbol){
            assert.equal(symbol, 'LUMI', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function(standard){
            assert.equal(standard, 'Lumi Token v1.0', 'has the correct standard');
        });
    });
